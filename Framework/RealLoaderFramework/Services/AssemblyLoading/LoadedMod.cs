    ...
    internal class LoadedMod {
        ...
        public void ModUnloaded(Task modTask) {
            if (!modTask.IsFaulted) {
                _logger.Info($"[{_clrMod.ModAttribute.ModName}] Mod was unloaded gracefully");
            }
            else {
                _logger.Warning($"[{_clrMod.ModAttribute.ModName}] Mod was unloaded with errors...\n" +
                    $"{modTask.Exception}");
            }
        ...
        public void Unload() {
            _logger.Warning($"[{_clrMod.ModAttribute.ModName}] Requested Unload.");
            ...
            Task.Factory.StartNew(() => {
                (_entryPointInstance as IRealLoaderMod).Unload();
                CancelTokenSource.Cancel();

                Task.Delay(TimeSpan.FromSeconds(5));

                if (!_runningTask.IsCompleted) {
                    _logger.Error($"[{_clrMod.ModAttribute.ModName}] Causing delinquent thread to hang!");
                }
        ...
        private void Load() {
            ...
            if (_entryPointInstance is null) {
                _logger.Error($"Could not load [{_clrMod.ModAttribute.ModName}]. Failed to activate/construct mod's entrypoint!");
                return;
            }

            (_entryPointInstance as IRealLoaderMod).Load();
            _logger.Debug($"[{_clrMod.ModAttribute.ModName}] has relinquished thread execution. Holding until unload event.");
